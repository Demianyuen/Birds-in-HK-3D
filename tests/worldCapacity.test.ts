...
it('admits 30 guests in one world, broadcasts all participants, and rejects guest 31', async () => {
  ...
  try {
    ...
    const results = await Promise.all(Array.from({ length: 30 }, (_, i) => join(i)));
    expect(results.filter(result => result === 'welcome')).toHaveLength(30);
    expect(identities.size).toBe(30);
    await expect.poll(() => maxSnapshot).toBe(30);
    expect(await join(31)).toBe('room_full');
  } finally {
